...
def train(model_name, training_type = 'K fold', K = 10, exclude_num = None, seed = 42, batch_size = 512, epochsNum = 5):
    ...
    if training_type == 'K fold':
        ...
        for fold_idx, (train_indices, test_indices) in enumerate(zip(train_folds, test_folds)):
            ...
            if model_name == 'Combined_CNN':
                ...
                combined_cnn.fit(train_ds,validation_data=val_ds,epochs=epochsNum,callbacks=call_backs)
            ### ProbeRating:
            if False:
